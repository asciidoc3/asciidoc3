...
class Trace(object):
    ...
    def __call__(self, name, before, after=None):
        ...
        the source text after substitutuion.
...
def filter_lines(filter_cmd, lines, attrs={}):
    ...
    if "pygmentize" in filter_cmd.strip().lower():
        if float(sys.version[:3]) < 3.3:
            message.stderr(
                """FAILED: 'pygments' requires Python 3.3 and higher!
                   Use 'source-highlight' or 'highlight' instead
                   - see manual chapter filter!""")
            sys.exit(1)
    try:
...
class Header:
    ...
    @staticmethod
    def parse():
        assert Lex.nxt() is Title and Title.level == 0
